...
const StatusUpdater = ({ issueId, currentStatus, onStatusUpdate, allowedStatuses }) => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          setLocationError('Unable to fetch location. Please enable location access in your browser.');
          setLocationLoading(false);
        },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Prepare update data with images and location
      const updateData = {
        issueId,
        status,
      ...
      if (onStatusUpdate) {
        try {
          ...
          // Call the parent component's update function with images and location
          const result = await onStatusUpdate(status, comment.trim() || `Status updated to: ${status}`, images, location.trim());
          ...
          // Fetch new location for next update
          ...
          if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(
              ...
              (error) => {
                setLocationError('Unable to fetch new location.');
                setLocationLoading(false);
              },
              { enableHighAccuracy: true, timeout: 10000, maximumAge: 0 }
            );
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-4">
          ...
          <input
            ...
            value={locationLoading ? 'Fetching location...' : location}
            readOnly
            disabled={isSubmitting}
          />
          {locationError && (
          ...
          {!locationLoading && !locationError && (
            <p className="text-xs text-gray-500 mt-1">GPS coordinates will be recorded with this update</p>
          )}
        </div>

        <div className="mb-4">
          <label htmlFor="images" className="block text-sm font-medium text-gray-700 mb-1">
            Upload Progress Images* (Required)
          </label>
          <input
            id="images"
            type="file"
            ...
            required
          />
          <p className="text-xs text-red-500 mt-1">* At least one image is required to show progress</p>
          <p className="text-xs text-gray-500">Max 5MB per image, JPG/PNG/WebP only</p>
        ...
        <div className="flex items-center justify-end">
          ...
          <button
            ...
            disabled={isSubmitting || locationLoading || currentStatus === status}
          >
            {locationLoading ? (
              <>
